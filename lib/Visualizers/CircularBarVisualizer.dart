...
class CircularBarVisualizer extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawCircle(new Offset(getWidth() / 2, getHeight() / 2),
        radius.toDouble(), wavePaint);
    if (waveData != null) {
      ...
      for (int i = 0; i < 120; i++, angle += 3) {
        ...
        points[i * 4 + 1] = getHeight() / 2 + radius * sin(radians(angle));

        points[i * 4 + 2] = getWidth() / 2 + (radius + t) * cos(radians(angle));
